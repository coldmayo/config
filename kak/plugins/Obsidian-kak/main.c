...
void add_workspace(char * path) {
	FILE * file = fopen("~/.config/kak/plugins/Obsidian-kak/workspaces.txt", "a");
	char line[PATH_MAX];
	strcat(line, vault_name(path));
	strcat(line, " | ");
	strcat(line, path);
	strcat(line, "\n");
	fputs(line, file);
